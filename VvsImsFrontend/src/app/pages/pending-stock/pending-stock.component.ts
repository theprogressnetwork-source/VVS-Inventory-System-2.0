...
@Component({
  ...
  styleUrl: './pending-stock.component.scss',
})
export class PendingStockComponent {
    ...
    this.API.getDataWithParams(this.config.GET_PRODUCTS)
      ...
      .subscribe({
        next: (data: any) => {
          ...
          this.listProducts = data.filter((x: any) => {
            const status = (x.orderStatus ?? '')
              .toString()
              .trim()
              .toLowerCase();
            return x.orderNo != null && status === 'pending';
          });
  ...
  initDataTable() {
    ...
    const savedColumns = this.persistence.loadColumnVisibility('pending_stock');
    this.allColumns = this.normalizeColumnVisibility(savedColumns);

    const savedSearch = this.persistence.loadSearchText('pending_stock');
    ...
    this.dataTable = $('#productsTable').DataTable({
      ...
      paging: true,
      info: true,
      responsive: false, // Disable responsive plugin for manual scroll control
      autoWidth: false, // Prevent auto width
      scrollX: true, // Enable scroll if needed
      scrollCollapse: true,
      search: {
    ...
    $('#productsTable').on('search.dt', () => {
      ...
      if (this.searchText !== searchValue) {
        ...
        this.searchDebounceTimer = setTimeout(() => {
          this.persistence.saveSearchText('pending_stock', this.searchText);
        }, 500);
      }
    });
  }
  ...
  openSettingsModal() {
    if (this.settingsModal) {
      ...
      this.settingsModal.componentName = 'Pending Inventory';
  ...
  resetColumnsOnly() {
    ...
    this.persistence.saveColumnVisibility('pending_stock', this.allColumns);
  ...
  resetSearchOnly() {
    this.searchText = '';
    this.persistence.saveSearchText('pending_stock', '');
  ...
  resetWidthsOnly() {
    this.toastr.info(
      'Column width reset not applicable for this table',
      'Info',
    );
  }

  resetSettings() {
    this.persistence.clearAllSettings('pending_stock');
  ...
  exportExcel() {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, 'Pending Inventory');

    // Export file
    XLSX.writeFile(workbook, 'Pending-Inventory.xlsx');
  }
  ...
  submitUpdateSingle(row: any) {
    if (row?.isSelling) return;

    if (!row?.orderNo || !row?.imei || !row?.newImei) {
      this.toastr.warning('OrderNo and IMEI are required', 'Validation');
      return;
    }
    ...
    console.log('Single IMEI Payload:', payload);

    this.isSubmitting = true;
    row.isSelling = true;

    this.API.postData(this.config.IMEI_UPDATE_SINGLE, payload)
      .pipe(
        finalize(() => {
      ...
      .subscribe({
        next: (res: any) => {
          ...
          // Prefer detailed per-item response if present
          const processed = Array.isArray(res.processed) ? res.processed : [];
          if (processed.length > 0) {
            const failed = processed.filter((r: any) => !r?.success);
            if (failed.length > 0) {
              const msg =
                failed[0]?.message ?? res?.message ?? 'IMEI update failed';
              this.toastr.error(msg, 'Error');
              return;
            }
            ...
            this.removeSoldRowFromTable(row);
            return;
          }

          // Fallback to top-level success boolean
          if (res?.success) {
            this.toastr.success(
              res?.message ?? 'IMEI updated successfully',
              'Success',
  ...
  private removeSoldRowFromTable(row: any) {
    const targetOrderNo = String(row?.orderNo ?? '').trim();
    const targetAttachedImei = String(row?.imei ?? '').trim();
    const rowIndex = this.rowData.findIndex(
      (r: any) =>
        r === row ||
        (String(r?.orderNo ?? '').trim() === targetOrderNo &&
          String(r?.imei ?? '').trim() === targetAttachedImei),
    );
    ...
    if (this.dataTable) {
      const nodeList = this.dataTable.rows().nodes().toArray();
      const rowNode = nodeList.find((node: any) => {
        const cells = (node as HTMLTableRowElement).cells;
        const orderNoText = cells?.[2]?.textContent?.trim() ?? '';
        const attachedImeiText = cells?.[4]?.textContent?.trim() ?? '';
        return (
          orderNoText === targetOrderNo &&
          attachedImeiText === targetAttachedImei
        );
      });

      if (rowNode) {
        this.dataTable.row(rowNode).remove().draw(false);
      } else {

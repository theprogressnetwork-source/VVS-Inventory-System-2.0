...
export class InputComponent {
  ...
  @ViewChild('bulkUploadModal') bulkUploadModal!: TemplateRef<any>;
  ...
  updateSkuList: any = [];
  vendors: any = [];
  searchText: string = ''; // For search persistence
  searchDebounceTimer: any; // For debounced saveFormGroup;
  ...
  ngOnInit(): void {
    ...
    const savedColumns = this.persistence.loadColumnVisibility('input');
    if (savedColumns) {
    ...
    this.searchText = this.persistence.loadSearchText('input');
  ...
  exportExcel() {
    ...
    // Prepare headers based on colDefs (skip Actions)
    const headers = this.colDefs.filter((col) => col.field);

    // Map rowData → formatted rows
    const formattedRows = this.listProducts.map((row: any) => {
      ...
      headers.forEach((col) => {
        ...
        // If valueGetter exists (e.g., calculated column)
        ...
        // Custom frontend formatting
        if (col.field === 'phoneCheck') value = value ? 'Pass' : 'Fail';
        if (col.field === 'dateAdded' && value)
          value = new Date(value).toLocaleDateString('fr-CA');

        // Use headerName as Excel column
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, 'Unsold Inventory');

    // Download
    XLSX.writeFile(workbook, 'Unsold-Inventory.xlsx');
  }
  ...
  editRecord(row: any): void {
    // row = event.node.data (filtered / searched row)
    this.isSaveMode = false;

    // Clear current form array
    this.products.clear();

    // Push the selected row
  ...
  updateProduct(): void {
    ...
    this.API.postData(this.config.UPDATE_PRODUCTS, payload, {
      ...
      'x-action-name': 'update-stock',
      'x-source-component': 'input',
      'x-entity-key': String(payload?.id ?? ''),
      'x-entity-sku': String(payload?.sku ?? ''),
    })
  ...
  onDeleteConfirmed() {
    ...
    this.API.removeData(this.config.DELETE_PRODUCT + this.stockId, {
      'x-module-name': 'inventory',
      'x-action-name': 'delete-stock',
      'x-source-component': 'input',
      'x-entity-key': String(this.stockId),
    })
  ...
  onSubmitOrder() {
    ...
    this.API.postData(this.config.SAVE_ORDER, payload)
  ...
  onColumnToggle(field: string, event: Event) {
    const checkbox = event.target as HTMLInputElement;
    const visible = checkbox.checked;

    // Update column visibility in colDefs
    ...
    // Save to persistence
    this.persistence.saveColumnVisibility('input', this.allColumns);
  ...
  onGridReady(params: GridReadyEvent) {
    ...
    const savedState = this.persistence.loadGridState('input');
  ...
  onQuickFilterChanged(value: string) {
    ...
    this.searchDebounceTimer = setTimeout(() => {
      this.persistence.saveSearchText('input', value);
    }, 500);
  }
  ...
  openBulkUploadModal() {
    this.selectedFile = null;
    this.modalRef = this.modalService.open(this.bulkUploadModal, {
      centered: false,
      backdrop: 'static',
      keyboard: false,
      size: 'lg',
  ...
  onSubmitBulkOrder() {
    ...
    // Use postDataFormBlob to handle potential file download
    this.API.postDataFormBlob(this.config.OUTGOING_PRODUCT, formData)
      .pipe(finalize(() => (this.bulkUploadloading = false)))
      .subscribe({
        next: (response: HttpResponse<Blob>) => {
          const body = response.body;

          // Helper to check if it's a JSON response
          const reader = new FileReader();
          reader.onload = () => {
            const text = reader.result as string;
            try {
              const json = JSON.parse(text);
              // Valid JSON response -> Success or known logic
              if (json && (json.status === 200 || json.success)) {
                this.toastr.success(json.message || 'Success', 'Success');
                this.getProducts();
                this.modalRef?.close();
              } else {
                // If it's valid JSON but not success, treat as error
                this.toastr.warning(json.message || 'Operation did not complete successfully', 'Alert');
              }
            } catch (e) {
              // Not JSON -> It's a file! Download it.
              this.downloadBlob(response);
            }
          };
          reader.readAsText(body as Blob); // Read first to sniff type
        },
        error: (error) => {
          // Error response is also a Blob initially
          if (error.error instanceof Blob) {
            ...
            reader.onload = () => {
              ...
              try {
                const json = JSON.parse(text);
                // Handle structured error JSON
                this.handleBulkError(json);
              } catch (e) {
                // If error body isn't JSON, it might be the error file itself (unlikely for 4xx/5xx but possible)
                // Just fallback to generic message
                this.toastr.error('An error occurred during upload', 'Error');
              }
            };
            reader.readAsText(error.error);
          } else {
            // Fallback for standard errors
            this.handleBulkError(error.error);
          }
        },
      });
  }
  ...
  downloadBlob(response: HttpResponse<Blob>) {
    ...
    // Detect filename
    let filename = 'error_list.csv'; // Default to CSV as the server returns CSV data
    ...
    // If filename has no extension, assume .csv if it's text/csv
    ...
    // Fix: If we force .xlsx on a CSV file, Excel throws an error. 
    // The server is returning CSV text, so we must ensure it's saved as .csv or .txt
    // If we detected .xlsx but the content type is text/csv, we might want to switch to .csv
    if (blob.type === 'text/csv' || blob.type === 'text/plain') {
    ...
    this.toastr.warning('Some records had errors. Please check the downloaded file.', 'Partial Success');
  }

  handleBulkError(error: any) {
    if (error) {
      if (error.message) {
  ...
  saveGridState() {
    if (this.gridApi) {
      ...
      this.persistence.saveGridState('input', state);
    }
  }
  ...
  updateColDefsFromSavedColumns() {
    this.colDefs = this.colDefs.map((col) => {
      const saved = this.allColumns.find((c) => c.field === col.field);
      if (saved) {
  ...
  getSettingsSummary(): SettingsSummary {
    ...
    const gridState = this.persistence.loadGridState('input');
  ...
  openSettingsModal() {
    if (this.settingsModal) {
      ...
      this.settingsModal.componentName = 'Input';
  ...
  resetSearchOnly() {
    this.searchText = '';
    this.persistence.saveSearchText('input', '');
   ...
   */
  resetWidthsOnly() {
    localStorage.removeItem('ims_input_grid_state');
   ...
   */
  resetSettings() {
    this.persistence.clearAllSettings('input');

...
export class GradesComponent {
  ...
  colDefs: ColDef[] = [
    ...
    { field: 'value', headerName: 'Grades', sortable: true, filter: true, },
  ...
  getModels() {
    this.isLoading = true;
    this.API.getData(this.config.GET_GRADES)
  ...
  submitData() {
    this.API.postData(this.config.SAVE_GRADES, this.modelForm.value).subscribe({
      next: (data: any) => {
        if (data.success) {
          this.getModels();
          this.modalRef?.close();
  ...
  updateData() {
    this.API.putData(this.config.SAVE_GRADES + '/' + this.modelForm.controls['id'].value, this.modelForm.value).subscribe({
      next: (data: any) => {
        if (data.success) {
          this.getModels();
          this.modalRef?.close();
  ...
  onDeleteConfirmed() {
    this.API.deleteData(this.config.SAVE_GRADES + '/' + this.modelForm.controls['id'].value).subscribe({
      next: (data: any) => {
        if (data.success) {
          this.getModels();
          this.modalRef?.close();

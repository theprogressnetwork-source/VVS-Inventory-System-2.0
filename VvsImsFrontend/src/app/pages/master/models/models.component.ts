...
export class ModelsComponent {
  ...
  colDefs: ColDef[] = [
    ...
    { field: 'value', headerName: 'Models', sortable: true, filter: true, },
  ...
  submitData() {
    this.API.postData(this.config.SAVE_MODELS, this.modelForm.value).subscribe({
      next: (data: any) => {
        if (data.success) {
          this.getModels();
          this.modalRef?.close();
  ...
  updateData() {
    this.API.putData(this.config.SAVE_MODELS + '/' + this.modelForm.controls['id'].value, this.modelForm.value).subscribe({
      next: (data: any) => {
        if (data.success) {
          this.getModels();
          this.modalRef?.close();
  ...
  onDeleteConfirmed() {
    this.API.deleteData(this.config.SAVE_MODELS + '/' + this.modelForm.controls['id'].value).subscribe({
      next: (data: any) => {
        if (data.success) {
          this.getModels();
          this.modalRef?.close();

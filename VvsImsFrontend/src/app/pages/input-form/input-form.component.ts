...
export class InputFormComponent {
  ...
  onImeiInput(index: number): void {
    ...
    let finalValue = cleanedParts.join(',');

    // Auto-add comma ONLY when typing forward
    const last = cleanedParts[cleanedParts.length - 1];
    if (last && last.length === 15 && !finalValue.endsWith(',')) {
      finalValue += ',';
    }
  ...
  save(): void {
    ...
    rows.forEach((row, rowIndex) => {
      const imeis = (row.imei || '')
        .split(',')
        .map((i) => i.trim())
        .filter((i) => /^\d{15}$/.test(i)); // ✅ strict 15 digit IMEI
    ...
    this.API.postData(this.config.SAVE_PRODUCTS, finalPayload, {
      'x-module-name': 'Input Inventory',
      'x-action-name': 'add-products',
      'x-source-component': 'input-form',
    })
  ...
  //     .subscribe({
  //       next: (res: any) => {
  //         // Logical failure from backend (success = false)
  //         if (res?.success === false) {
  //           this.toastr.error(res.message || 'Operation failed.', 'Error');
  //           return;
  //         }
  ...
  //         this.toastr.error('Unexpected server response.', 'Error');
  //       },

  //       error: (err) => {
  //         // API / Network / Validation Errors
  ...
  //           // Validation errors (Laravel / ASP.NET style)
  //           if (err.error.errors) {
